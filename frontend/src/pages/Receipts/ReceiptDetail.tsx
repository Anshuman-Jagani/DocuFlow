...
const ReceiptDetail: React.FC = () => {
  ...
  const fetchReceipt = async () => {
    try {
      ...
      if (receiptData.document?.id) {
        try {
          const docResponse = await api.get(`/api/documents/${receiptData.document.id}/download`, {
            responseType: 'blob',
          });
    ...
    <DashboardLayout>
      <div className="max-w-5xl mx-auto space-y-6">
        <button onClick={() => navigate('/receipts')}
          className="flex items-center gap-2 text-[#888888] hover:text-white transition-colors text-sm font-medium">
        ...
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[#888888]">{new Date(receipt.purchase_date!).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={handleDownload}
              className="p-2 text-[#888888] hover:text-white bg-[#0A0A0A] border border-[#1A1A1A] rounded-lg hover:bg-[#111111] transition-colors" title="Download Scan">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-[#0A0A0A] border border-[#1A1A1A] rounded-2xl overflow-hidden">
              <div className="p-6 border-b border-[#1A1A1A] flex justify-between items-center bg-[#111111]">
                <span className="text-[10px] font-bold text-[#888888] uppercase tracking-widest">Transaction Details</span>
                <span className={`px-2 py-1 rounded text-xs font-bold border ${receipt.is_business_expense ? 'bg-[#0A0A0A]/10 text-white border-white/20' : 'bg-[#1A1A1A] text-[#888888] border-[#1A1A1A]'}`}>
                  {receipt.is_business_expense ? 'Business' : 'Personal'}
                </span>
              </div>
              <div className="p-6 grid grid-cols-2 sm:grid-cols-4 gap-6">
                <div>
                  <p className="text-xs text-[#888888] mb-1 flex items-center gap-1"><CreditCard className="w-3 h-3" /> Method</p>
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-[#111111]">
                    <tr>
                      <th className="px-6 py-3 text-left text-[10px] font-bold text-[#888888] uppercase tracking-widest">Item</th>
                      <th className="px-6 py-3 text-center text-[10px] font-bold text-[#888888] uppercase tracking-widest">Qty</th>
                      <th className="px-6 py-3 text-right text-[10px] font-bold text-[#888888] uppercase tracking-widest">Price</th>
                      <th className="px-6 py-3 text-right text-[10px] font-bold text-[#888888] uppercase tracking-widest">Total</th>
                    </tr>
                  </thead>
          ...
          <div className="lg:col-span-1">
            <div className="bg-[#0A0A0A] border border-[#1A1A1A] rounded-2xl overflow-hidden sticky top-6">
              ...
              <div className="p-4 bg-[#111111]">
                {documentBlobUrl ? (
                  <img src={documentBlobUrl} alt="Receipt Scan"
                    className="w-full rounded-lg border border-[#1A1A1A]" />
                ) : (
                  <div className="aspect-[3/4] flex flex-col items-center justify-center text-[#888888] gap-2 italic">
                    <AlertCircle className="w-8 h-8" />
                    <p className="text-xs">{receipt.document ? 'Loading scan...' : 'Image unavailable'}</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} title="Delete Receipt">
          <div className="space-y-4 pt-2">
            <p className="text-[#888888] text-sm">Are you sure you want to remove this record? This will delete the extracted data and the attached scan.</p>
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowDeleteModal(false)}
                className="px-4 py-2 text-sm font-bold text-[#888888] border border-[#1A1A1A] rounded-lg hover:bg-[#111111] hover:text-white transition-colors">
                Cancel
              </button>
