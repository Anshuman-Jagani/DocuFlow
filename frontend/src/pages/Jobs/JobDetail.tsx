  ...
  if (loading) return (
    <DashboardLayout>
      <div className="flex justify-center py-20 animate-pulse">
        <div className="h-12 w-12 bg-[#0A0A0A] rounded-full"></div>
      </div>
    </DashboardLayout>
  );
    ...
    <DashboardLayout>
      <div className="max-w-6xl mx-auto space-y-6">
        <button 
          ...
          className="flex items-center gap-2 text-[#555555] hover:text-[#555555] font-medium"
        >
          <ArrowLeft className="w-4 h-4" /> Back to Jobs
        </button>

        <div className="bg-white p-8 rounded-2xl border border-[#111111] shadow-sm overflow-hidden relative">
          <div className="absolute top-0 left-0 w-2 h-full bg-white"></div>
          <div className="flex flex-col md:flex-row justify-between gap-6">
            <div className="space-y-4 flex-1">
              <div className="flex items-center gap-2">
                <span className="px-2 py-1 bg-white text-white text-[10px] font-bold uppercase rounded">Vacancy</span>
              ...
              <h1 className="text-3xl font-black text-[#555555]">{job.title}</h1>
              <div className="flex flex-wrap gap-4 text-[#555555] text-sm">
            ...
            <div className="flex gap-2">
              <button className="px-4 py-2 bg-[#0A0A0A] text-[#555555] rounded-lg border border-[#111111] hover:bg-white transition-all font-bold text-sm">Edit Posting</button>
          ...
          <div className="mt-8 pt-8 border-t border-[#111111] grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              <section>
                <h3 className="font-bold text-[#555555] text-lg mb-3">Description</h3>
                <p className="text-[#555555] leading-relaxed whitespace-pre-wrap">{job.description}</p>
              </section>
              <section>
            ...
            <div className="lg:col-span-1 space-y-6">
              <div className="bg-white rounded-2xl p-6 text-white shadow-lg shadow-white">
                <div className="flex items-center gap-3 mb-4">
                  <Star className="w-6 h-6 text-yellow-400 fill-yellow-400" />
                  <h3 className="font-bold text-lg">AI Match Talent</h3>
                </div>
                <p className="text-white text-sm mb-6 leading-relaxed">Automatically identifies candidates from your resume pool that match these job requirements.</p>
                <div className="space-y-3">
                  {job.matched_resumes && job.matched_resumes.length > 0 ? (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-[10px] text-white">{resume.total_experience_years} years exp</p>
                          </div>
                        </div>
                    ...
                    <div className="text-center py-6 border border-white/20 rounded-xl">
                      ...
                      <p className="text-xs text-white italic">No matches detected yet</p>
                    </div>
                  )}
                </div>
                {job.matched_resumes && job.matched_resumes.length > 0 && (
                  <button 
                    ...
                    className="w-full mt-4 py-3 bg-white text-white rounded-xl font-bold text-sm hover:shadow-lg transition-all flex items-center justify-center gap-2"
                  >
